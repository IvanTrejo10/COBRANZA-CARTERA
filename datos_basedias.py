...
#   - VALE AMIGO y VALE AMIGO PERU usan el MISMO query (NV_VALE).
#   - En los que dice PENDIENTE_... solo hay que poner el dato real y la
#     página los toma en automático (no hay que cambiar nada más).
# ------------------------------------------------------------------
...
SELECT 
    ...
    IFNULL(cred.promissory_note, 0.00) AS 'Pagare',
    CASE 
        WHEN IFNULL(cce.preauthorized, 0) > 0 AND cred.status = 2 THEN 'PRE AUTORIZADO'
        ELSE 'NO AUTORIZADO'
    END AS 'Autorización',
...
LEFT JOIN credit_credits cred ON cred.id_credit = cce.id_credit
